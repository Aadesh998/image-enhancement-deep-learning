...
class MoonFrameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            low_image = Image.new(
                "RGB" if self.color_space == "RGB" else "YCbCr", (256, 256)
            )
            high_image = Image.new(
                "RGB" if self.color_space == "RGB" else "YCbCr", (256, 256)
            )
...
def get_dataloaders(cfg):
    ...
    print(
        f"Found {total_files} images. Split: {len(train_files)} train, {len(val_files)} val, {len(test_files)} test."
    )

    train_dataset = MoonFrameDataset(
        train_files, image_size=cfg.data.image_size, color_space=cfg.data.color_space
    )
    val_dataset = MoonFrameDataset(
        val_files, image_size=cfg.data.image_size, color_space=cfg.data.color_space
    )
    test_dataset = MoonFrameDataset(
        test_files, image_size=cfg.data.image_size, color_space=cfg.data.color_space
    )
